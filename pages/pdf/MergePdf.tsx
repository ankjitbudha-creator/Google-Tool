...
export const MergePdf: React.FC = () => {
        ...
        for (const file of Array.from(fileList)) {
            if (file.type === 'application/pdf') {
                try {
                    ...
                    setMessage({type: 'error', text: `Could not process ${file.name}`});
                }
            }
        }
    ...
    const mergePdfs = useCallback(async () => {
        ...
        try {
            ...
            setMessage({ type: 'error', text: 'An error occurred while merging the PDFs.' });
        } finally {
            setIsProcessing(false);
        }
    }, [files]);
    ...
    return (
        <div className="space-y-4">
            ...
            {isProcessing && <Spinner text={message?.text || 'Processing...'} />}
